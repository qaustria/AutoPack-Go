...
if [ "$i" -ge 20 ]; then
    echo "Cone started, but its private healthz did not report version 1.4.6." >&2
...
case "$health" in
    *'"version":"1.4.6"'*) ;;
    *)
        echo "nginx is running, but the proxied healthz did not report version 1.4.6." >&2
        journalctl -u nginx -n 30 --no-pager >&2 || true
        exit 1
        ;;
esac

echo "Cone 1.4.6 is running through nginx on localhost:80."
